...
**D.** Classification metrics for self:self, male:male, male:female cell type lookup, using different molecule types and k-mer sizes. x-axis, k-mer size in DNA space, i.e. a k-mer size of 21 corresponds to a an amino acid length of 7. y-axis, value of classification metric. Upper left, adjusted rand index; upper right, F1 score; lower left, recall; lower right, precision.
**E.** Mean of classification metrics across all three mouse:mouse experiments, highlighting the k-mer size that maximizes the mean for each metric and alphabet. Vertical lines, standard deviation of the mean. x-axis, k-mer size in DNA space, i.e. a k-mer size of 21 corresponds to a an amino acid length of 7. y-axis, value of classification metric. Left, adjusted rand score; right, F1 score.
**F.** Molecules and k-mer sizes maximizing classification metrics from (**E**).
**G-I.** Confusion matrices of classification with best performing parameters from (**E**). x-axis, predicted cell label; y-axis, ground truth cell label.
**G.** DNA alphabet with k-mer size of 42.
**H.** Protein 20-letter alphabet with k-mer size of 27.
**I.** Dayhoff 6-letter amino acid encoding with k-mer size of 39.
...
](images/SVG/supplemental_figure1.svg){#sfig1:sfig1 tag="supplemental_figure1" width="100%"}
