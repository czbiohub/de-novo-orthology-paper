...
Nullomers are sequences that are not present in the genome annotation or protein sequences.
While this seems like there are a contraditionc btwween wanting to use all the data, and then only using the data that is present in the database annotation, we show we are able to predict protein coding reads using simulated data.
The signal of a non-protein coding sequence is very obvious.
There are only so many k-mers that are compatible with eukaryotic, or mammalian life.
There are amino acid combinations that are not seen int he natural world.
