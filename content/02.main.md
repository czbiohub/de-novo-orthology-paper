...
We found that by searching for the number of k-mers overlapping the query cell, using a containment search [cite sourmash gather paper?], we were able to correctly identify the cell type ---% of the time (Fig. {@fig:fig2}B).
There were minimal misclassifications and most misclassificaitons occur as a result of ---
Curious whether these cell types had any en4riched sequences that weren't present in the annotated genomes, we then performed differential k-mer expression to identify enriched sequences in the cell types.
We found that compared to gene expresion, we had a ---% overlap with the genes that are known to be enriched by gene expression of 1:1 orthologs, and we found --- genes that were enriched in the cell types but were not 1:1 orthologs, and thus would have been missd by traditional methods.
