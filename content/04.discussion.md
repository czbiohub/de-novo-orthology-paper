...
The recently published SAMmap algorithm [@https://doi.org/10.1101/2020.09.28.317784] showed the ability of cross-species cell type annotation. However, one of its limitations is thhat it still requires a well annotated genome, with genes identified, in each species.
This method allows for studying of isster species from a genus or family, by mapping the reads to a somewhat related organism, but still using all the reads that pass the bar of looking like protein-coding reads.
The advantages of `kmermaid` are that it does not require a well annotated genome, and opens up single-cell atlas generation to the 99.99% of animal that have no reference genome.
